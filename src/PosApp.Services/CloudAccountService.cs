...
public sealed class CloudAccountService : ICloudAccountService
{
    ...
    public async Task TestConnectionAsync(CancellationToken cancellationToken = default)
    {
        ...
        using var response = await Http.GetAsync($"{endpoint}/v1/health", cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var identity = await _credentials.GetOrCreateDeviceIdentityAsync(cancellationToken);
        var deviceName = NormalizeDeviceName(identity.DeviceName);
    ...
    private static string ConfiguredEndpoint()
        => NormalizeEndpoint(CloudDeploymentConfiguration.GetRequiredEndpoint());

    private static string NormalizeDeviceName(string? detectedName)
    {
        var value = detectedName?.Trim() ?? string.Empty;
        if (value.Length > 100) value = value[..100];
        return value.Length == 0 ? "Windows POS" : value;
    }

    private static string CurrentVersion()
        => Assembly.GetEntryAssembly()?.GetName().Version?.ToString(3) ?? "1.10.13";
...
internal static class CloudDeploymentConfiguration
{
    private const string MetadataKey = "PosAppCloudApiUrl";

    public static string? TryGetEndpoint()
    {
        var rawValue = Assembly.GetEntryAssembly()?
...
internal sealed class CloudCredentialStore
{
    ...
    public async Task<CloudCredential?> LoadAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var credential = JsonSerializer.Deserialize<CloudCredential>(clearBytes, JsonOptions);
            var configuredEndpoint = CloudDeploymentConfiguration.TryGetEndpoint();
            if (credential != null && configuredEndpoint != null)
                credential.Endpoint = configuredEndpoint;
            return credential;
        }
