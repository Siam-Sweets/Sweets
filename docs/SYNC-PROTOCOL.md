...
The same `RequiresReconciliation` gate protects both an existing local database being linked for the first time and a staged local restore. Restore publication itself is local and all-or-nothing: PosApp drains synchronization during shutdown, validates a private staged copy, checkpoints and exclusively locks the outgoing SQLite file set, preserves the old main database, removes its WAL/SHM/journal sidecars, and atomically replaces the main file. If another process still owns any file, the pending backup and current database remain untouched for a later retry. On startup after a successful restore, PosApp recovers the encrypted account scope from the retained safety database when needed, resets the pull cursor, and blocks both upload and download. The administrator must explicitly choose server state, or validate that the cloud organization is empty before uploading local data as a new state. **Use server** clears synchronized operational rows and sync metadata in dependency-safe order before replaying cloud state; local users are retained only for cached PIN identity. Older local rows are never silently pushed over newer cloud versions.
