...
- The browser account portal uses the same tenant-scoped authentication endpoints and permissions as the desktop. It keeps rotating tokens and the matching current-tab device ID in browser session storage, renews expired access tokens, revokes its server session on sign-out, and serves a nonce-restricted content security policy. Creating another organization generates a new device UUID instead of reusing a device registered to another tenant. Non-secret identifier/device mappings remain in local storage so later sign-in can reuse the correct registration; tokens do not. Separate tabs cannot mix a refresh token with another organization's active device ID. The portal never receives Turso or Cloudflare credentials.

## Desktop secrets

Only access/refresh tokens are present on a desktop. `DpapiTokenStore` encrypts them with Windows Data Protection API `CurrentUser` scope and atomically replaces a profile-specific `cloud-session.dat`. The backward-compatible `legacy` profile keeps `%LOCALAPPDATA%\PosApp\Security\cloud-session.dat`; added profiles use `%LOCALAPPDATA%\PosApp\Profiles\<random-profile-id>\Security\cloud-session.dat`. Copying either file to another Windows account does not make it usable. Organization/store/device metadata is non-secret.

The local organization selector at `%LOCALAPPDATA%\PosApp\Profiles\profiles.json` contains only random profile IDs, tenant/organization/store display metadata, username display metadata, and timestamps. It never contains a password, PIN, token, device secret, Turso credential, or Worker secret. Each profile resolves to its own SQLite database, device UUID, backup/restore area, and update migration-state records. PosApp changes the selector only at a restart boundary, so an active EF Core context cannot cross from one tenant cache to another.
...
Backups and restore initiation remain local. Restore validates the staged database before touching live data, waits for exclusive main/WAL/SHM/journal access, preserves the outgoing database, and removes old sidecars before atomic publication. A sharing conflict leaves the live database and staged backup untouched. The reconciliation state prevents restored data from bypassing cloud versions. The rotating desktop cloud diagnostic log records correlation/stage data, queue counts, structured codes, request IDs, and sanitized exception metadata, while redacting or excluding credentials, tokens, payloads, SQL, customer fields, and direct contact details. Operational policy should retain local PosApp logs and database backups securely, review diagnostic lines before sharing them, and restrict Windows account access.
