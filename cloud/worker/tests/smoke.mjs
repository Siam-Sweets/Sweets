  ...
  const results = body.requests.map((request) => {
    ...
    if (sql.startsWith("SELECT d.id, d.name, d.platform")) {
      return executeResult(
        ...
        [
          [textCell("device-one"), textCell("Front POS"), textCell("Windows"), textCell("1.9.5"),
            textCell("2026-07-22T00:00:00.000Z"), textCell("2026-07-22T01:00:00.000Z"), nullCell(), integerCell(1)],
          [textCell("device-two"), textCell("Back POS"), textCell("Windows"), textCell("1.9.5"),
...
try {
  ...
  assert.equal((await health.json()).version, "1.9.5");
