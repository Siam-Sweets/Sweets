...
4. Test `<worker-url>/v1/health`; it should return `ok: true` and version `1.10.10`.
...
- Do not add `ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION`; v1.10.10 no longer depends on Node.js 20.
...
v1.10.10 uses 100,000 PBKDF2-HMAC-SHA256 iterations for cloud owner passwords so signup works in the Cloudflare Workers runtime.
