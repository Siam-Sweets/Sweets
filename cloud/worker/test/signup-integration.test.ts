...
const env: Env = {
  ...
  API_VERSION: "1",
  DEPLOYMENT_VERSION: "2.0.18-test",
};
...
describe("organization provisioning integration", () => {
  it("renders the public status page with an account-creation result", async () => {
    ...
    expect(response.headers.get("x-posapp-deployment-version")).toBe("2.0.18-test");
    ...
    expect(html).toContain('name="posapp-deployment-version" content="2.0.18-test"');
  ...
  it("renders the tenant-scoped account portal at the Worker root", async () => {
    ...
    expect(html).toContain('name="posapp-portal-version" content="2.0.18-test"');
  });
