...
| **Products & Inventory** | Full CRUD for products and categories, SKU/barcode tracking, stock levels, low-stock alerts, stock adjustments (purchase, wastage, manual), stock movement history, stock valuation at cost |
...
- **QRCoder** (for future QR receipt codes)
- **LiveCharts2 / SkiaSharp** for charts (referenced for report visualizations)
...
- Role-based access:
  ...
  - **Manager**: + Products, Inventory, Customers, Reports
...
**EF Core migration errors on first run** — delete `%LOCALAPPDATA%\PosApp\posapp.db` and re-launch; the seeder will recreate everything.
